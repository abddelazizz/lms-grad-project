...
const Signup = () => {
  ...
  const handleResendVerification = async () => {
    setIsResending(true);
    try {
      const response = await axios.post('http://localhost:5000/api/auth/resend-verification', { email: unverifiedEmail });
      toast.success(response.data.message);
    } catch (err) {
      ...
      <div className="container d-flex justify-content-center align-items-center">
        <div className="signup-form-card bg-white p-4 p-md-5 rounded-4 border border-light-subtle shadow-soft w-100" style={{maxWidth: '480px'}}>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <button type="button" className="btn btn-social-signup w-100 py-2 rounded-2 d-flex align-items-center justify-content-center gap-2 mb-4 mb-md-5 fs-7 border border-light-subtle">
            ...
            <div className="text-center">
              <p className="text-dark mb-0 fs-7">Already have an account? <a href="/login" className="text-dark fw-bold text-decoration-none ms-1">Login <i className="fa-solid fa-arrow-right ms-1 x-small"></i></a></p>
            </div>
          </form>
        </div>
      </div>
