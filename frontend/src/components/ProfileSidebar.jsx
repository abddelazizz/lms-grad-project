...
const ProfileSidebar = () => {
  ...
  return (
    <aside 
      className={`sidebar-right-profile ${isCollapsed ? 'collapsed' : ''}`}
    ...
    >
      ...
      >
        <i className={`fas fa-chevron-${isCollapsed ? 'left' : 'right'}`} style={{ fontSize: '11px', color: '#31506a' }}></i>
      </button>

      {/* Mini Icons for Collapsed State */}
      {isCollapsed && (
        <div className="d-flex flex-column align-items-center gap-4 mt-5 pt-3">
          <div className="rounded-circle overflow-hidden shadow-sm" style={{ width: '45px', height: '45px', border: '2px solid #fff' }}>
            <img src={user?.picture || `https://ui-avatars.com/api/?name=${encodeURIComponent(user?.name || 'User')}`} className="w-100 h-100 object-fit-cover" alt="avatar" />
          </div>
          
          <button className="btn btn-white rounded-circle p-0 border-0 shadow-sm d-flex align-items-center justify-content-center" style={{ width: '40px', height: '40px', backgroundColor: '#f8fafc' }} onClick={(e) => { e.stopPropagation(); handleToggleDropdown(); }}>
            <i className="far fa-bell text-muted" style={{ fontSize: '18px' }}></i>
            {unreadCount > 0 && <span className="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger" style={{ fontSize: '8px' }}>{unreadCount}</span>}
          </button>

          <Link to={role === 'instructor' ? "/instructor/my-courses" : "/dashboard/my-courses"} className="text-muted d-flex align-items-center justify-content-center" onClick={(e) => e.stopPropagation()} style={{ width: '40px', height: '40px' }}>
            <i className="fas fa-book-open" style={{ fontSize: '18px' }}></i>
          </Link>

          <div className="text-muted d-flex align-items-center justify-content-center" style={{ width: '40px', height: '40px' }}>
      ...
      <button className="sidebar-close-btn" onClick={(e) => {
        e.stopPropagation();
        document.querySelector('.sidebar-right-profile').classList.remove('show-mobile');
      ...
      {!isCollapsed && (
        <>
          <div className="profile-header d-flex justify-content-between align-items-center mb-4 profile-content-to-hide" style={{ marginTop: '40px' }}>
          ...
          <div className="profile-main-info text-center mb-4 position-relative">
            <div className="profile-avatar-large mx-auto mb-4 shadow-sm p-1 bg-white" style={{ width: '110px', height: '110px', borderRadius: '50%', border: '1px solid #f1f5f9', transition: 'all 0.4s ease' }}>
            ...
            <div className="profile-content-to-hide">
          ...
          <div className="mb-5 px-1 profile-content-to-hide">
          ...
          {role !== 'admin' && (
            <div className="mt-auto pt-4 profile-content-to-hide">
              <div className="d-flex justify-content-between align-items-center mb-4">
